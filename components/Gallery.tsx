    ...
    <div className="max-w-7xl mx-auto relative">
        <div className="grid lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-8 relative flex justify-center h-[500px] md:h-[600px]">
            <AnimatePresence initial={false} custom={direction} mode="wait">
              ...
              >
                <motion.div 
                  ...
                  className="antique-frame max-w-full relative h-fit group lg:cursor-none"
            ...
            <div className="absolute inset-y-0 left-2 lg:-translate-x-1/2 flex items-center">
              <button onClick={prevSlide} className="w-10 h-10 rounded-full bg-white/80 backdrop-blur-sm text-zen-stone border border-zen-beige shadow-lg hover:bg-zen-gold hover:text-white transition-colors flex items-center justify-center z-40">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M15 19l-7-7 7-7" /></svg>
              </button>
            </div>
            <div className="absolute inset-y-0 right-2 lg:translate-x-1/2 flex items-center">
              <button onClick={nextSlide} className="w-10 h-10 rounded-full bg-white/80 backdrop-blur-sm text-zen-stone border border-zen-beige shadow-lg hover:bg-zen-gold hover:text-white transition-colors flex items-center justify-center z-40">
...
const ImmersionMode: React.FC<ImmersionModeProps> = ({ art, onClose }) => {
    ...
    return (
        <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[100] bg-black flex items-center justify-center overflow-hidden lg:cursor-none"
            onClick={onClose}
            onMouseMove={handleMouseMove}
            onTouchStart={() => setShowMagnifier(true)}
            onTouchEnd={() => setShowMagnifier(false)}
                ...
                <div className="flex items-center space-x-8">
                    {art.status !== 'vendu' && (
                      <motion.button 
                          ...
                          className="px-10 py-4 bg-zen-gold text-white text-[10px] uppercase tracking-[0.3em] font-bold hover:bg-white hover:text-zen-gold transition-all duration-500 shadow-xl"
                      >
                          {t('gallery.priceInquiry', { defaultValue: 'Demander le prix' })}
                      </motion.button>
                    )}
